...
1. 介绍
   ...
   2. 特权软件栈术语
      ...
      - sbi_query_memory：输入hart id和内存块信息的指针info，将内存大小写入info指针，写入成功返回0,写入失败返回非0.
      - sbi_hart_id：无输入，返回硬件线程的id，是一个32位无符号长整型。
      - sbi_num_harts
      - sbi_timebase：无输入，返回CPU的工作频率，是一个32位无符号长整型。
      - sbi_set_timer：输入64位无符号整型(unsigned long long)，设置计时器，无返回。
      - sbi_send_ipi
      - sbi_clear_ipi
      - sbi_shutdown
      - sbi_console_putchar：输入字符c，在控制台打印出此字符，无返回。
      - sbi_console_getchar
...
4. 管理员级ISA
   ...
   1. 管理员CSR：
      ...
      - sstatus: 管理员状态寄存器，追踪处理器当前的操作状态，是XLEN位R/W寄存器。
        - SPP位记录了进入管理员模式前硬件线程的特权级，如果来自于用户模式则置0,其它模式则置1.
        - SIE位开启或停止管理员模式下的所有中断。
        - SPIE位指示了在进入管理员模式前管理员中断是否开启。当内陷进入管理员模式，SPIE设置为SIE，SIE设置为0。当执行SRET指令，SIE设置为SPIE,SPIE设置为1.
        - UIE位开启或停止用户模式的中断。
        - UPIE位的功能和SPIE位类似。
   ...
   5. Sv32:基于页面的32位虚拟存储器系统
      ...
      Sv32是用32位虚拟地址管理34位物理地址，如果我没有理解错误的话，它可以管理16G的内存！

      1. 通过页表基址寄存器sptbr和va找到页目录的入口pte: sptbr + va.vpn[1] * 4
      2. 如果 (pte.v==0) 或 (pte.r==0 && pte.w==1)，停止并报告一个页地址异常。
      3. 如果(pte.r==0 && pte.x==0)，表明还存在下级页表，则下级页表的地址：pte.ppn*4K，需继续计算直到找到一个物理页或出现页错误。如果(pte.r==1 || pte.x==1)，表明找到一个物理页，则进行下一步。
      4. 通过pte的特权位判断访问是否允许，不允许则报告一个页错误，允许则pa.pgoff = va.pgoff。
